...
PROGVERSIONS="$PWD"/sofware-versions-used.txt;
		...
		PROG1=$(conda list | grep '^samtools' | sed 's/  */;/g' | cut -f2 -d';');
			...
			echo -e "script: 04_mapping_VirCap.sh" >> "$PROGVERSIONS";
...
##  activate the environment for this downstream analysis
eval "$(conda shell.bash hook)";
conda activate NGS-kma;
		ENV1=();
		ENV1=NGS-kma;
		PROG2=();
		PROG2=$(conda list | grep '^kma' | sed 's/  */;/g' | cut -f2 -d';');

			echo -e "conda environment=$ENV1" >> "$PROGVERSIONS";			
			echo -e "programm\tversion" >> "$PROGVERSIONS";
			echo -e "kma\t$PROG2" >> "$PROGVERSIONS";
			echo -e "all scripts used will be documented in this file" >> "$PROGVERSIONS";
			echo -e "\n" >> "$PROGVERSIONS";

while getopts "w:x:y:z:a:b:c:d:e:f:m:r:s:t:" opt; do
  case $opt in
     w)
...
DB1k='/mnt/lely_DB/minikraken2/viral_20200519/viral_20200519_kma/viral_20200519_kma';
DB2k='/mnt/lely_DB/Old_Shared_DATABASES/VIRUSES/VirCap/kma/';
...
DB2='/mnt/lely_DB/Old_Shared_DATABASES/VIRUSES/VirCap/VirCapSeq_targets.fa';
DBshort2='vircap';

# output files
	RPKM1="$MAPPING"/"$SAMPLE"."$DBshort1".RPKM.tab;
	REFSTATS1="$MAPPING"/"$SAMPLE"."$DBshort1".refstats.tab;
	COVSTATS1="$MAPPING"/"$SAMPLE"."$DBshort1".covstats.tab;
	SCAFSTATS1="$MAPPING"/"$SAMPLE"."$DBshort1".scafstats.tab;	
	OUTM1="$TMP"/"$SAMPLE"."$DBshort1".sam;
	OUTMb1="$TMP"/"$SAMPLE"."$DBshort1".bam;
	OUTU1="$TMP"/"$SAMPLE"."$DBshort1".unmapped.fastq;
	
	RPKM2="$MAPPING"/"$SAMPLE"."$DBshort2".RPKM.tab;
	REFSTATS2="$MAPPING"/"$SAMPLE"."$DBshort2".refstats.tab;
	COVSTATS2="$MAPPING"/"$SAMPLE"."$DBshort2".covstats.tab;
	SCAFSTATS2="$MAPPING"/"$SAMPLE"."$DBshort2".scafstats.tab;	
	OUTM2="$TMP"/"$SAMPLE"."$DBshort2".sam;
	OUTMb2="$TMP"/"$SAMPLE"."$DBshort2".bam;
	OUTU2="$TMP"/"$SAMPLE"."$DBshort2".unmapped.fastq;
	

# map against database 1
		bbmap.sh -Xmx48g ref="$DB1" build=1 in1="$R1" in2="$R2" outu="$OUTU1" outm="$OUTM1" "$TAGS" refstats="$REFSTATS1" covstats="$COVSTATS1" rpkm="$RPKM1" scafstats="$SCAFSTATS1" untrim=t showprogress=0 ambig=best slow=t=f sam=1.3 fast=t ow >> "$LOG1" 2>&1 ;

# optional output takes very long time for processing
		samtools view -@ "$NODES" -S "$OUTM1" -b -o "$OUTMb1";
		samtools sort -@ "$NODES" "$OUTMb1" -o "$MAPPING"/"$SAMPLE"."$DBshort1"_sorted.bam;
		samtools index "$MAPPING"/"$SAMPLE"."$DBshort1"_sorted.bam "$MAPPING"/"$SAMPLE"."$DBshort1"_sorted.bai;

# map against database 2
		bbmap.sh -Xmx48g ref="$DB2" build=1 in1="$R1" in2="$R2" outu="$OUTU1" outm="$OUTM2" "$TAGS" refstats="$REFSTATS2" covstats="$COVSTATS2" rpkm="$RPKM" scafstats="$SCAFSTATS2" untrim=t showprogress=0 ambig=best slow=t=f sam=1.3 fast=t ow >> "$LOG1" 2>&1 ;

# optional output takes very long time for processing
		samtools view -@ "$NODES" -S "$OUTM2" -b -o "$OUTMb2";
		samtools sort -@ "$NODES" "$OUTMb2" -o "$MAPPING"/"$SAMPLE"."$DBshort2"_sorted.bam;
		samtools index "$MAPPING"/"$SAMPLE"."$DBshort2"_sorted.bam "$MAPPING"/"$SAMPLE"."$DBshort2"_sorted.bai;
...
# kma variables
	KMAout1="$MAPPING"/"$SAMPLE"."$DBshort1".tab;
	KMAout2="$MAPPING"/"$SAMPLE"."$DBshort2".tab;
	MP=20;

			kma -t "$NODES" -ipe "$R1" "$R2" -o "$KMAout1" -t_db "$DB1k" -matrix -nc -mp "$MP" -fpm p  -ConClave 2 -cge -dense -ef -1t1 -ca -boot;
			kma -t "$NODES" -ipe "$R1" "$R2" -o "$KMAout2" -t_db "$DB2k" -matrix -nc -mp "$MP" -fpm p  -ConClave 2 -cge -dense -ef -1t1 -ca -boot;
...
# output files
	RPKM1="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort1".RPKM.tab;
	REFSTATS1="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort1".refstats.tab;
	COVSTATS1="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort1".covstats.tab;
	SCAFSTATS1="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort1".scafstats.tab;	
	OUTM1="$TMP"/"$SAMPLE"."$short".clean."$DBshort1".sam;
	OUTMb1="$TMP"/"$SAMPLE"."$short".clean."$DBshort1".bam;
	OUTU1="$TMP"/"$SAMPLE"."$short".clean."$DBshort1".unmapped.fastq.gz;
	
	RPKM2="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort2".RPKM.tab;
	REFSTATS2="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort2".refstats.tab;
	COVSTATS2="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort2".covstats.tab;
	SCAFSTATS2="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort2".scafstats.tab;	
	OUTM2="$TMP"/"$SAMPLE"."$short".clean."$DBshort2".sam;
	OUTMb2="$TMP"/"$SAMPLE"."$short".clean."$DBshort2".bam;
	OUTU2="$TMP"/"$SAMPLE"."$short".clean."$DBshort2".unmapped.fastq.gz;
...
# kma variables
	KMAout1="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort1".tab;
	KMAout2="$MAPPING"/"$SAMPLE"."$short".clean."$DBshort2".tab;
	MP=20;
...
#
#
# KMA enables alignment towards databases, using two k-mer mapping steps and one alignment step.
#     Options are:      Desc:
...
#
#KMA-1.4.9 maps and/or aligns raw reads to a template database.
